...
<body>
    ...
    <?php
        ...
        if (isset($_SESSION['pz_uid'])) { # 로그인 했다면 이라는
          $email = $_SESSION['pz_uid'];
          $uname = $_SESSION['pz_uname'];
          //echo "$uname 환영합니다.";
          $login = true; # 트루면 로그인 되어있따
    ...
    if($result->num_rows > 0) { # 검색된 레코드가 있으면, 이라는 조건문
        while($row = $result->fetch_assoc()) { # 레코드 한 개를 연관배열 형태로 가져옴
            // echo $row['id']." : ".$row['name']." : ".$row['large']." : "
            //     .$row['small']." : <img src='images/".$row['photo']."'<br>";
            ?>
